 ...
 * @brief 运行脚本，返回脚本执行内容
 * @param e
 * @param full_path，脚本路径
 * @return
 */
evm_val_t evm_run_script(evm_t * e, const char * full_path);
/**
 * @brief 运行字符串，指定一种语言
 * @param e
 * @param string，字符串
 ...
 * @return
 */
evm_val_t evm_run_eval(evm_t * e,  evm_val_t *object, char * string,int lang_type);
/**
 * @brief 加载模块
 * @param e，虚拟机
 * @param path，模块文件全路径
 * @return
 */
evm_val_t evm_module_load(evm_t * e, const char * full_path);
/*** 虚拟机相关函数 ***/
/**
 * @brief 虚拟机启动
 * @param e，虚拟机参数
