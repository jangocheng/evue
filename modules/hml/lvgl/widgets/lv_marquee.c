 ...
 *      INCLUDES
 *********************/
#include "lv_marquee.h"
#if LV_USE_LABEL != 0
#include "src/lv_core/lv_obj.h"
#include "src/lv_misc/lv_debug.h"
#include "src/lv_core/lv_group.h"
#include "src/lv_draw/lv_draw.h"
...
static void lv_marquee_refr_text(lv_obj_t * label);
static void lv_label_revert_dots(lv_obj_t * label);

#if LV_USE_ANIMATION
    static void lv_marquee_set_offset_x(lv_obj_t * label, lv_coord_t x);
    static void lv_marquee_set_offset_y(lv_obj_t * label, lv_coord_t y);
#endif

static bool lv_marquee_set_dot_tmp(lv_obj_t * label, char * data, uint32_t len);
static char * lv_marquee_get_dot_tmp(lv_obj_t * label);
...
lv_obj_t * lv_marquee_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    if(copy == NULL) {
        ...
        lv_marquee_set_long_mode(new_label, LV_LABEL_LONG_EXPAND);
        lv_marquee_set_text(new_label, "Text");
    }
    /*Copy 'copy' if not NULL*/
    else {
        lv_label_ext_t * copy_ext = lv_obj_get_ext_attr(copy);
        lv_marquee_set_long_mode(new_label, lv_label_get_long_mode(copy));
        lv_marquee_set_recolor(new_label, lv_label_get_recolor(copy));
        lv_marquee_set_align(new_label, lv_label_get_align(copy));
        if(copy_ext->static_txt == 0)
            lv_marquee_set_text(new_label, lv_label_get_text(copy));
        else
            lv_marquee_set_text_static(new_label, lv_label_get_text(copy));
        ...
        if(copy_ext->dot_tmp_alloc && copy_ext->dot.tmp_ptr) {
            ...
            lv_marquee_set_dot_tmp(new_label, ext->dot.tmp_ptr, len);
        }
        else {
...
void lv_marquee_set_long_mode(lv_obj_t * label, lv_label_long_mode_t long_mode)
{
    ...
    lv_anim_del(label, (lv_anim_exec_xcb_t)lv_marquee_set_offset_x);
    lv_anim_del(label, (lv_anim_exec_xcb_t)lv_marquee_set_offset_y);
#endif
    ext->offset.x = 0;
    ext->offset.y = 0;
...
void lv_marquee_set_recolor(lv_obj_t * label, bool en)
{
    ...
    lv_marquee_refr_text(label); /*Refresh the text because the potential color codes in text needs to
 ...
 */
void lv_marquee_get_letter_pos(const lv_obj_t * label, uint32_t char_id, lv_point_t * pos)
{
    LV_ASSERT_OBJ(label, LV_OBJX_NAME);
    LV_ASSERT_NULL(pos);

    const char * txt         = lv_marquee_get_text(label);
    lv_label_align_t align = lv_marquee_get_align(label);
 ...
 */
uint32_t lv_marquee_get_letter_on(const lv_obj_t * label, lv_point_t * pos_in)
{
    LV_ASSERT_OBJ(label, LV_OBJX_NAME);
...
void lv_marquee_cut_text(lv_obj_t * label, uint32_t pos, uint32_t cnt)
{
    ...
    char * label_txt = lv_marquee_get_text(label);
...
static lv_design_res_t lv_label_design(lv_obj_t * label, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    ...
    if(mode == LV_DESIGN_COVER_CHK)
        ...
        label_draw_dsc.sel_start = lv_marquee_get_text_sel_start(label);
        label_draw_dsc.sel_end = lv_marquee_get_text_sel_end(label);
...
static lv_res_t lv_label_signal(lv_obj_t * label, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_STYLE) {
        ...
        info->result = lv_marquee_get_style(label, info->part);
...
#if LV_USE_BIDI
        if(ext->static_txt == 0) lv_marquee_set_text(label, NULL);
#endif
    }
...
static void lv_marquee_refr_text(lv_obj_t * label)
{
    ...
    /*In roll mode keep the size but start offset animations*/
        ...
        if(size.x > lv_area_get_width(&txt_coords)) {
            ...
            lv_anim_set_exec_cb(&a, (lv_anim_exec_xcb_t)lv_marquee_set_offset_x);
            lv_anim_set_time(&a, lv_anim_speed_to_time(ext->anim_speed, a.start, a.end));
            lv_anim_set_playback_time(&a, a.time);

            lv_anim_t * anim_cur = lv_anim_get(label, (lv_anim_exec_xcb_t)lv_marquee_set_offset_x);
            int32_t act_time = 0;
        ...
        if(size.y > lv_area_get_height(&txt_coords) && hor_anim == false) {
            ...
            lv_anim_set_exec_cb(&a, (lv_anim_exec_xcb_t)lv_marquee_set_offset_y);
            lv_anim_set_time(&a, lv_anim_speed_to_time(ext->anim_speed, a.start, a.end));
            lv_anim_set_playback_time(&a, a.time);

            lv_anim_t * anim_cur = lv_anim_get(label, (lv_anim_exec_xcb_t)lv_marquee_set_offset_y);
            int32_t act_time = 0;
            bool playback_now = false;
            if(anim_cur) {
                act_time = anim_cur->act_time;
    ...
    /*In roll inf. mode keep the size but start offset animations*/
        ...
        if(size.y <= lv_area_get_height(&txt_coords)) { /*No dots are required, the text is short enough*/
            ...
            uint32_t letter_id = lv_marquee_get_letter_on(label, &p);
            ...
            if(lv_marquee_set_dot_tmp(label, &ext->text[byte_id_ori], len)) {
                for(i = 0; i < LV_LABEL_DOT_NUM; i++) {
...
static void lv_label_revert_dots(lv_obj_t * label)
{
    ...
    uint8_t i      = 0;
    char * dot_tmp = lv_marquee_get_dot_tmp(label);
